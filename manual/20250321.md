...
function App() {
  ...
  const [ toDo, setToDo] = useState('');  // 얘네가 element
  const [ toDos, setToDos] = useState([]);  // 얘가 배열
...
- toDos의 element들의 자료형 string = toDo의 자료형과 동일(input에서 썼기 때문에, input type="text")
- setToDo(''); 가 일어나는 시점 -> if문 이후(setToDos() 실행 이후 -> toDos에 새로운 상태인 toDo가 업데이트 된 이후)
- setTodos(); 가 실행되어야만 하는 시점(1,2,3,4 로 주석에 표시해뒀고, 답은 3이었습니다)
...
currentArray가 배열이라고 가정했을 떄, ... currentArray는 currentArray의 element에 해당합니다.
그래서 App.jsx를 참조했을 때
...
이상의 경우 속성명이 5개쯤 있다고 봤을 때, 속성명1과 속성명2에 해당하는 값만 바꾸고, 속성명 3,4,5,의 속성값은 그대로 유지한다는 의미입니다.
...
[<li>0번지</li>, <<li>1번지</li>, <li>2번지</li>, <li>3번지</li>, <li>4번지</li>]
