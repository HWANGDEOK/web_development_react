...
이런 문제를 해결하기 위해 리액트 내장 Hook인 ContextAPI나 redux, recoil과 같은 상태 관리 라이브러리를 사용합니다(전에 말한 적은 있습니다).
...
useContext는 전역적으로 데이터를 공유하기 때문에 중간 다리 역할만하는(여기서 Main.jsx) 컴포넌트를 건너 뛰고 데이터가 직접적으로 필요한 컴포넌트에서 바로 사용이 가능하도록 합니다. 따라서 이상의 모든 하위 컴포넌트에 props가 전달되는 문제를 피할 수 있습니다.
...
git commit -m "feat: context API apllied kpop list started"

다하면 App.jsx에 프래그먼트만 남기고 초기화

src -> components -> Button.jsx / KPopList.jsx / Main.jsx
src -> contexts 폴더 -> context.jsx
App.css 파일에 kpoplist의 App.css 코드 그대로 붙여넣기
