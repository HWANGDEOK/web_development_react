...
그리고 또한 영화의 제목을 클릭했을 때 -> 다른 페이지로 인동을 할 수 있어야합니다.
...
근데 여태까지 저희가 배운 방식에서 (특히 web3 수업) 클릭을 통한 페이지 이동을 하는 방식으로는
'a' 태그를 이용했었습니다.
...
```
a태그로 h2 태그를 감싸주는 형태를 생각할 수 있습니다(페이지 상에서 제목을 클릭한다는 점에서 그 위치가 Home / App 컴포넌트가 아니라 Movie 컴포넌트에 있다는 점을 명시해야합니다.)
...
import { BrowserRouter as Router, Switch, Route } from "react-router-dom";를 import 했습니다.
...
-> 브라우저의 새로고침이 없이도 다른 페이지로 이동을 시켜주는 컴포넌트 -> Movie 컴포넌트에 Link를 적용시킬겁니다(정확하게는 얘가 a태그를 대체한다고 보면됩니다.)

react-router-dom 5버전으로 생성했었는데 7버전이 깔려서 수정 예정
...
현재 저희 프로젝트의 경우에는 두 개의 Route를 사용
하나는 Home으로 이동하기 위함
...
설치하고 npm run dev를 통해서 재실행하게 됐을 때 제대로 되는 지 확인해야함.
...
react-router-dom v5에서 Switch 컴포넌트를 사용할 떄, Route가 선언된 순서대로 매칭되기 때문에 더 구체적인 경로("/movie/:id")가 더 일반적인 경로("/")보다 위에 위치해야 함.
...
git commit -m "feat: 20250325 movieapp Link"