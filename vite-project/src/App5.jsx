...
/**
 * 1. Counter 컴포넌트 생성하고 초기값이 0인 count 상태를 선언.
 */
// function Counter(){
...
//     <div>
//       <p>현재 카운트 값 = {count}</p>
//       <button onClick={() => setCount(count+10)}>증가시킵니다</button>
//     </div>
//   )
// }
...
// 틀린 예시로 실행을 경우 생기는 문제점 :

/**
 * 이벤트 핸들러(onClick) 내에서 함수를 직접 호출했을 경우 컴포넌트가 렌더링 될 때 함수가 호출되어 무한 루프가 발생할 수 있음.
 * 
 * setCount의 경우
 * const [const, setCount] = userState(initalValue)로 정의돼있을 뿐인데,
 ...
 * 
 * 그렇기에 따로 setter 형태로 함수를 정의하지 않더라도
 ...
 *  결과 - 버튼을 클릭할 때만 증가됨
 * 즉시 실행 함수 :
 *  전달 방식 - 함수의 실행결과값을 전달(렌더링 할 대마다 실행)
 *  실행 시점 - 컴포넌트가 렌더링 될 때
 *  결과 - 렌더링이 멈추지 않기 때문에 계속해서 함수를 호출하여 무한 루프
 */

/**
 * 이상까지 작성했을 때 App.jsw의 button과 비교를 해보면 이제 하나만 차이가 납니다. 저희가 작성한 방식은 call3() 유형으로 매개변수는 없는데, return은 있는 형태였습니다.
 * 
 * 그런데 App.jsx를 확인해봤을 때 setCount((count) => count+1)
 * 으로 작성되어 call4()유형(매개변수o / return 0)으로 작성되었음을 확인할 수 있습니다.
 *  
 * 
 * 이상의 차이가 나는 이유는 리액트의 성격 때문인데,
 * 상태 업데이트는 비동기적으로 일어납니다. 새 상택 값이 현재 상태 값에
